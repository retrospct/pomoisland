...
The Settings "Behavior" and "Sounds" sections expose several OS-level capabilities:
launch-at-login, always-on-top, magnetic snap, hide-during-screen-sharing, pause-when-idle,
global shortcut, system notifications, and packaged alarm sounds. Implementing all of them
faithfully (screen-share detection, system idle polling, login items, global hotkey capture)
is a large surface with platform edge cases. The agreed scope for this pass is the **core
experience first**.
...
- **Wired for real:** always-on-top, magnetic snap (in-app window drag), tray, and a basic
  Web-Audio completion chime with volume.
- **Persisted as prefs, no OS behavior yet (no-ops):** launch-at-login, hide-during-screen-
  sharing, pause-when-idle, global shortcut, native notifications, and real alarm sound files.
